...
resource "aws_route_table" "private01" {
  count = length(var.private_subnet_cidr_blocks01)
...
resource "aws_subnet" "private01" {
  ...
  cidr_block        = var.private_subnet_cidr_blocks01[count.index]
  availability_zone = var.availability_zones[0]

  tags = merge(
    {
      Name        = var.private_subnet_names_01[count.index],
      Project     = var.project,
      Environment = var.environment
    },
    var.tags
...
resource "aws_subnet" "private02" {
  ...
  vpc_id            = aws_vpc.dishvpc.id
  //cidr_block        = "${cidrsubnet(var.cidr_block, 8, count.index)}"
  cidr_block        = var.private_subnet_cidr_blocks02[count.index]
  availability_zone = var.availability_zones[1]

  tags = merge(
    {
      Name        = var.private_subnet_names_02[count.index],
      Project     = var.project,
      Environment = var.environment
    },
    var.tags
